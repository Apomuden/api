...
class AccessController extends Controller
{
    ...
    function login(LoginRequest $request)
    {
        try {
            ...
            if ($user->status == 'ACTIVE' && !Security::confirmPassword($request->password, $user->password, $user->id)) {
            ...
            } elseif ($user->status == 'RECOVERY_MODE') {
                ...
                } elseif (date('Y-m-d H:i:s', strtotime($passwordReset->expiry_date)) < now()) {
                    return ApiResponse::withNotAuthorized('Account recovery expired!');
                } elseif (!Security::confirmPassword($request->password, $passwordReset->token, $user->id)) {
