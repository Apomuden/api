...
class EreceiptRequest extends ApiFormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            'payment_channel_id'=>['bail',Rule::requiredIf(function() use($id){
                   return $id!=null;
            }),$this->softExists('payment_channels','id')],
            'cheque_no' => ['bail',Rule::requiredIf(function(){
                return ucwords($this->payment_channel->name)=='Cheque';
            ...
            'payee_transaction_id'=>['bail',Rule::requiredIf(function() use($id){
                return $id && in_array($this->payment_channel->name, ['MTN Mobile Money', 'AirtelTigo Money', 'Vodafone Cash', 'G-Money']);
            }),$this->softUnique('ereceipts', 'payee_transaction_id', $id)],
            'payee_account_no'=>['bail',Rule::requiredIf(function() use($id){
                return $id && trim(request('cheque_no')) && request('cheque_no')!=null||in_array($this->payment_channel->name,['MTN Mobile Money', 'Cheque','CHEQUE', 'AirtelTigo Money', 'Vodafone Cash', 'G-Money']);
            })],
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            if(!(isset($all['bank_id']) && $all['bank_id'])  && in_array(ucwords($this->payment_channel->name), ['Bank Interface', 'Banking Interface', 'Bank Deposit']))
            $validator->errors()->add('bank_id', 'bank id is required!');

            else if((isset($all['cheque_no']) && $all['cheque_no'])){
                if (ucwords($this->payment_channel->name) != 'Bank Deposit')
