...
class ServiceOrderRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        return [
            ...
            'funding_type_id'=> 'bail|'. ($id ? 'sometimes' : 'required').'|exists:funding_types,id',
            'billing_system_id'=> 'bail|'. ($id ? 'sometimes' : 'required').'|exists:billing_systems,id',
            'billing_cycle_id'=>'bail|'. ($id ? 'sometimes' : 'required'). '|exists:billing_cycles,id',
            'payment_style_id'=>'bail|'. ($id ? 'sometimes' : 'required'). '|exists:payment_styles,id',
            'payment_channel_id'=>'bail|'. ($id ? 'sometimes' : 'required'). '|exists:payment_channel,id',
            'insured'=>'bail|sometimes|boolean',
            'billing_sponsor_id'=>'bail|'. ($id ? 'sometimes' : 'required'). '|exists:billing_sponsors,id',
            'sponsorship_policy_id'=>'bail'. ($id ? 'sometimes' : 'required').'|exists:sponsorship_policies,id',
