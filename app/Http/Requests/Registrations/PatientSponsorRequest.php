...
class PatientSponsorRequest extends ApiFormRequest
{
    ...
    public function rules()
    {
        ...
        $data= [
            ...
            'reg_no' => 'bail|'. ($id  ? 'sometimes|nullable' : 'required').'|'.($benefit_type=='BABY'? $this->softUniqueWith('patient_sponsors', 'reg_no,patient_id', $id): $this->softUnique('patient_sponsors', 'reg_no',$id)),
            'card_serial_no' => 'bail|' . ($id ? 'sometimes' : 'required') . '|'.($benefit_type=='BABY'? $this->softUniqueWith('patient_sponsors', 'card_serial_no,patient_id', $id): $this->softUnique('patient_sponsors', 'card_serial_no', $id)),
